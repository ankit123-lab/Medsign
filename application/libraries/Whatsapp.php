...
class Whatsapp {
	...
	public function send_test_message($data) {
		if($this->_sms_send_from == 1) { //1=Send whatsapp message from Twilio
			$this->twilio($data);
		}
	}
	...
	private function twilio($data) {
        ...
        if(!empty($data['doctor_id']) && empty($data['is_promotional']))
	...
	public function update_log($data) {
		if(!empty($data['SmsSid'])) {
			...
			if(!empty($data['ErrorCode']) && $data['ErrorCode'] == "63003") {
				$phone_number = substr($data['To'], -10);
				if(!empty($phone_number)) {
					$user_data = ['wa_optout' => 1];
					$user_where = ['user_phone_number' => $phone_number];
					$this->CI->Common_model->update('me_users', $user_data, $user_where);
				}
			}
		}
	}

	public function add_whatsapp_comein_log($data) {
		if(!empty($data['SmsSid'])) {
			$phone_number = substr($data['From'], -10);
			$insert_data = [
			...
			$this->CI->Common_model->insert('me_message_tracking', $insert_data);
		}
	}
}