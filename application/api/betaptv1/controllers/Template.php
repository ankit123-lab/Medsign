...
Class Template extends MY_Controller {
    ...
    public function get_template_post() {
        ...
        try {
            ...
            $search_where = '';
            if (!empty($search)) {
                $search_where = " AND template_diagnosis_name LIKE '%" . $search . "%' ";
            }

            $get_template_query = "SELECT 
                                        template_id,
                                    ...
                                    LEFT JOIN
                                        " . TBL_TEMPLATE_DRUG . " 
                                    ON 
                                        template_id = template_drug_binding_template_id 
                                    LEFT JOIN
                                        " . TBL_DRUGS . " 
                                    ON 
                                        template_drug_binding_drug_id = drug_id
                                    LEFT JOIN
                                        " . TBL_CLINICAL_NOTES . "
                                    ON
                                        FIND_IN_SET(clinical_notes_catalog_id, template_clinical_notes_id) 
                                    AND 
                                        clinical_notes_catalog_status=1
                                    WHERE
                                        template_doctor_id ='" . $doctor_id . "' 
                                    AND
                                        template_status = 1 " . $search_where . " 
                                    GROUP BY 
                                        template_id";

            $get_template_data = $this->Common_model->get_all_rows_by_query($get_template_query);
