...
class MY_Model extends CI_Model {
     ...
     * @return type
     */
    function get_single_row($table_name, $fields, $where = array(), $join = array(), $join_type = '', $like_array = array(), $or_where = array(), $where_in = array()) {
		...
		if(isset($where_in) && is_array($where_in) && count($where_in) > 0) {
			foreach ($where_in as $key => $valArray) {
				$this->db->where_in($key, $valArray);
            }
		}
     ...
     * @return integer
     */
    function get_count($table_name, $fields, $where = array(), $join = array(), $order_by = array()) {
        $this->db->select($fields);
        if (is_array($join) && count($join) > 0) {
            foreach ($join as $key => $value) {
                $this->db->join($key, $value);
	...
	/**
     * 
     * This function is help to update multiple recored in database
     * 
     * @param string $table_name
     * @param array $data
     * @return boolean
     */
    function update_multiple($table_name, $data, $pKey) {
		if(!empty($pKey)){
			$this->db->update_batch($table_name, $data, $pKey);
			return true;
		}else{
			return true;
		}
