...
class Drug extends MY_Controller {
    ...
    public function get_drugs_post() {
        try {
            ...
            $drug_generic_id = !empty($this->post_data['drug_generic_id']) ? trim($this->Common_model->escape_data($this->post_data['drug_generic_id'])) : "";
            $doctor_id = !empty($this->post_data['doctor_id']) ? trim($this->Common_model->escape_data($this->post_data['doctor_id'])) : "";
            if(empty($brand_name) && empty($drug_generic_id))
                $this->bad_request();
            $get_drug_list = $this->drug->search_drug($brand_name, $doctor_id, $drug_generic_id);
            if (!empty($get_drug_list)) {
                $this->my_response['status'] = true;
                $this->my_response['messgae'] = lang('common_detail_found');
                $this->my_response['data'] = $get_drug_list;
                $this->my_response['rows'] = $rows;
            } else {
                $this->my_response['status'] = false;
    ...
    public function search_generic_post() {
        try {
            $generic_name = !empty($this->post_data['generic_name']) ? trim($this->Common_model->escape_data($this->post_data['generic_name'])) : "";
            $doctor_id = !empty($this->post_data['doctor_id']) ? trim($this->Common_model->escape_data($this->post_data['doctor_id'])) : "";
            $result = $this->drug->search_generic($generic_name, $doctor_id);
            if (!empty($result)) {
                $get_generic = $this->drug->get_generic("drug_generic_id,drug_generic_title");
                $generics = array_column($get_generic, 'drug_generic_title', 'drug_generic_id');
                foreach ($result as $key => $value) {
                    $drug_generic_id_arr = explode(',', $value['drug_drug_generic_id']);
                    $generic_title = [];
                    $result[$key]['generic_count'] = count($drug_generic_id_arr);
                    foreach($drug_generic_id_arr as $generic_id){
                        if(!empty($generics[$generic_id])) {
                            $generic_title[] = $generics[$generic_id];
                        }
                    }
                    usort($generic_title, function($a, $b) {
                        return $a <=> $b;
                    });
                    $result[$key]['generic_title'] = implode(' + ', $generic_title);
                }
                usort($result, function($a, $b) {
                    return $a['generic_count'] <=> $b['generic_count'];
                });
                $generics_list = array();
                $temp_arr = array();
                $last_count = false;
                foreach ($result as $key => $value) {
                    if($last_count != $value['generic_count']) {
                        if($last_count) {
                            usort($temp_arr, function($a, $b) {
                                return $a['generic_title'] <=> $b['generic_title'];
                            });
                            $generics_list = array_merge($generics_list, $temp_arr);
                            $temp_arr = array();
                        }
                        $last_count = $value['generic_count'];
                    }
                    $temp_arr[] = $value;
                }
                $generics_list = array_merge($generics_list, $temp_arr);
                $this->my_response['status'] = true;
                $this->my_response['messgae'] = lang('common_detail_found');
                $this->my_response['data'] = $generics_list;
            } else {
                $this->my_response['status'] = false;
                $this->my_response['messgae'] = lang('common_detail_not_found');
                $this->my_response['data'] = array();
    ...
    public function get_related_drugs_post() {
        try {
            ...
            $get_related_data_sql = " SELECT 
                                        ...
                                        drug_drug_generic_id as drug_generic_id, 
                                        drug_drug_unit_id as drug_unit_id 
                                        FROM 
                                        " . TBL_DRUGS . " 
										WHERE 
                                        drug_id =" . $drug_id . " AND 
										drug_status = 1 AND drug_user_id IS NULL ";
            $get_related_data = $this->Common_model->get_single_row_by_query($get_related_data_sql);
			if(empty($get_related_data['drug_strength']) || empty($get_related_data['drug_generic_id']) || empty($get_related_data['drug_unit_id'])) {
				$this->my_response['status'] = false;
				$this->my_response['messgae'] = lang('common_detail_not_found');
				$this->my_response['data'] = array();
			} else {
				//get related drugs
				$get_related_drugs_sql = " SELECT " . $columns . " FROM " . TBL_DRUGS . " WHERE drug_id != " . $drug_id." AND drug_preferred_brand IS NOT NULL AND drug_preferred_brand <= 5 ";
				if (!empty($get_related_data['drug_strength']))
					$get_related_drugs_sql .= " AND drug_strength IN ('" . $get_related_data['drug_strength'] . "') ";
				if (!empty($get_related_data['drug_generic_id']))
					$get_related_drugs_sql .= " AND drug_drug_generic_id = '" . $get_related_data['drug_generic_id'] . "' ";
				if (!empty($get_related_data['drug_unit_id']))
					$get_related_drugs_sql .= " AND drug_drug_unit_id IN (" . $get_related_data['drug_unit_id'] . ") ";
				$get_related_drugs_sql .= " ORDER BY drug_preferred_brand LIMIT 0,5 ";
    ...
    public function get_drug_brand_type_post() {
        $drug_unit_is_display = !empty($this->post_data['drug_unit_is_display']) ? trim($this->Common_model->escape_data($this->post_data['drug_unit_is_display'])) : "";
        try {

            $columns = "drug_unit_id, 
                        drug_unit_name,
                        drug_unit_medicine_type  AS medicine_type,
                        CONCAT(drug_unit_medicine_type, ' (',drug_unit_name,')') AS drug_unit_medicine_type";
            ...
            $get_brand_type_list_query = "SELECT 
                                    ...
                                    WHERE 
                                        drug_unit_status = 1";
            if($drug_unit_is_display != "")
                $get_brand_type_list_query .= " AND drug_unit_is_display=" . $drug_unit_is_display;

            if(IS_FILE_CACHING_ACTIVE == 1){
                create_cache_sub_dir('me_drug_units');
                $hashObj = sha1($get_brand_type_list_query);
                ...
                if(!$resultCached = $this->cache->file->get($hashObj)){
                    ...
                    $this->cache->file->save($hashObj, $get_brand_type_list, CACHE_TTL);
                }else{
                    $get_brand_type_list = $resultCached;
                }
            }else{
                $get_brand_type_list = $this->Common_model->get_all_rows_by_query($get_brand_type_list_query);
            }
        ...
        try {
            ...
            if(IS_FILE_CACHING_ACTIVE == 1){
                ...
                if(!$resultCached = $this->cache->file->get($hashObj)){
                    ...
                    $this->cache->file->save($hashObj, $get_drug_generic, CACHE_TTL);
                }else{
                    $get_drug_generic = $resultCached;
                }
            }else{
                $get_drug_generic = $this->Common_model->get_all_rows_by_query($query);
            }
    ...
    public function get_drug_frequency_post() {
        ...
        try {
            ...
            $query .= " ORDER BY drug_frequency_order_by ASC";

            if(IS_FILE_CACHING_ACTIVE == 1){
                create_cache_sub_dir('me_drug_frequency');
                $hashObj = sha1($query);
                $this->load->driver('cache');
                if(!$resultCached = $this->cache->file->get($hashObj)){
                    $get_drug_frequency = $this->Common_model->get_all_rows_by_query($query);
                    $this->cache->file->save($hashObj, $get_drug_frequency, CACHE_TTL);
                }else{
                    $get_drug_frequency = $resultCached;
                }
            }else{
                $get_drug_frequency = $this->Common_model->get_all_rows_by_query($query);
            }
            ...
            if (is_array($drug_data) && count($drug_data) > 0) {
                foreach ($drug_data as $data) {
                    if (!isset($temp_array[$data['drug_name']])) {
                        $drug_frequency_value = NULL;
                        if($data['default1'] != '0' || $data['default2'] != '0' || $data['default3'] != '0') {
                            $drug_frequency_value = $data['default1'].'-'.$data['default2'].'-'.$data['default3'];
                        }
                        $insert_drug[] = array(
                            'drug_name' => strtoupper($data['drug_name']),
							'drug_name_with_unit' => strtoupper($data['drug_name'].' ('.$data['drug_unit_medicine_type'].')'),
                            ...
                            'drug_frequency_value' => $drug_frequency_value,
    ...
    public function get_doctor_drug_post() {
        try {
            ...
            $get_drug_list = $this->drug->get_doctor_drug($where_array);
            $count = $this->drug->get_doctor_drug($where_array, true);
    ...
    public function add_prescription_by_patient_post() {
        try {
            ...
            $insert_prescription = array(
                ...
                'patient_prescription_doctor_name' 	=> $doctor_name,
