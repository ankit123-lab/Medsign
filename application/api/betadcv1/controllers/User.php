...
class User extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model(array("User_model", 'Doctor_model', 'Advertisement_model', 'Auditlog_model'));
        ...
        try {
            ...
            if ($sening_sms) {
                ...
                if ($insert_id > 0) {
					...
					if(isset($user_data['temp_auth_code'])){ unset($user_data['temp_auth_code']); }
        ...
        try {
            ...
            if ($login_with == 1 && !empty($phone_number)) {
                ...
                if (!empty($user_details) && count($user_details) > 0) {
					if (in_array($user_details['user_status'], [1,3])) { //allowed Active / Pending status
                ...
                if (!empty($result) && count($result) > 0) {
					...
					if (in_array($result['user_status'], [1,3])) { //allowed Active / Pending status
                            ...
                            if ($enable_2_way_authentication == 2) {
                                ...
                                if ($token) {
                                    ...
                                    if ($user_type == 1) {
                                        ...
                                        $get_user_details['doctor_id'] = $result['user_id'];
                                        if(!empty($doctor_clinic_mapping['doctor_clinic_mapping_doctor_id'])) {
                                            $get_user_details['doctor_id'] = $doctor_clinic_mapping['doctor_clinic_mapping_doctor_id'];
                                        }
                                        ...
                                        $get_user_details['is_sub_active'] = is_sub_active($get_user_details['doctor_id']);
                                        $get_user_details['hour_format'] = get_hour_format($get_user_details['doctor_id']);
                                        $get_user_details['sub_plan_setting'] = doctor_sub_plan_setting($get_user_details['doctor_id']);
                                        if(!$get_user_details['is_sub_active'] && $get_user_details['user_type'] != 1) {
                                            $this->my_response = array(
                                                "status" => false,
                                                "message" => lang("subscription_expired_message")
                                            );
                                            $this->send_response();
                                        }
                                    }
                                    ...
                                    //Get Doctor advertisement
                                    $advertisement = $this->get_advertisement_data($get_user_details['user_id']);
                                    if(count($advertisement) > 0) {
                                        $get_user_details['advertisement_data'] = $advertisement;
                                        $get_user_details['next_request_time'] = (strtotime($advertisement[0]['advertisement_end_date']) - time()) * 1000;
                                    }
    ...
    public function get_advertisement_post() {
        try {
            ...
            if(is_numeric($doctor_id)) {
                $advertisement = $this->get_advertisement_data($doctor_id);
                if(count($advertisement) > 0) {
                    $this->my_response = array(
                        "status" => true,
                        "message" => "success",
                        "advertisement_data" => $advertisement,
                        "next_request_time" => (strtotime($advertisement[0]['advertisement_end_date']) - time()) * 1000
                    );
                } else {
                    $this->my_response = array(
                        "status" => false,
    ...
    /**
     * Description :- This function is used to get the advertisement as per condition
     */
    private function get_advertisement_data($doctor_id) {
        ...
        if ($get_user_details->doctor_clinic_mapping_role_id == 1 && $get_user_details->user_status == 1) {
            ...
            $advertisement = $this->Advertisement_model->get_advertisement($doctor_id, $start_date, $end_date, $doctor_specialisation_ids, $doctor_qua_ids);
            $result = array();
            foreach ($advertisement as $key => $value) {
                if(empty($result[$value['advertisement_id']])) {
                    $result[$value['advertisement_id']] = array(
                        'advertisement_id' => $value['advertisement_id'],
                        'advertisement_title' => $value['advertisement_title'],
                        'advertisement_descriptions' => $value['advertisement_descriptions'],
                        'advertisement_rotate_time' => $value['advertisement_rotate_time'],
                        'advertisement_image_rotate_timing' => $value['advertisement_image_rotate_timing'],
                        'advertisement_start_date' => $value['advertisement_start_date'],
                        'advertisement_end_date' => $value['advertisement_end_date']
                    );
                }
                if(empty($result[$value['advertisement_id']]['advertisement_type_data']))
                    $result[$value['advertisement_id']]['advertisement_type_data'] = array();
                    $result[$value['advertisement_id']]['advertisement_type_data'][] = array(
                    'advertisement_type_id' => $value['advertisement_type_id'],
                    'advertisement_type' => $value['advertisement_type'],
                    'advertisement_filepath' => get_file_full_path($value['advertisement_filepath']),
                    'advertisement_text' => $value['advertisement_text'],
                    'advertisement_videourl' => $value['advertisement_videourl'],
                ...
                if (strtotime($get_auth_details['auth_otp_expiry_time']) >= $this->utc_time) {
                    if ($get_auth_details['auth_code'] == $otp) {
                        ...
                        if ($is_auth_updated > 0) {
                            ...
                            $columns = implode(',',array_keys($update_user_number));
                            $user_old_data = $this->User_model->get_details_by_id($get_auth_details['auth_user_id'],$columns);
                            
                            $this->Auditlog_model->create_audit_log($get_auth_details['auth_user_id'], 2, AUDIT_SLUG_ARR['USER_PROFILE_ACTION'], $user_old_data, $update_user_number, TBL_USERS, 'user_id', $get_auth_details['auth_user_id']);
    ...
    public function register_resend_otp_post() {
        ...
        try {
            ...
            if (
                ...
                $message = sprintf(OTP_MESSAGE, $otp);
                $send_otp = array(
                    'phone_number' => $user_data['temp_user_phone_number'],
                    'is_not_check_setting_flag' => true,
                    'is_stop_sms' => IS_STOP_SMS,
                    'is_stop_whatsapp_sms' => IS_STOP_WHATSAPP_SMS,
                    'message' => $message,
                ...
                $whatsapp_data = [
                    'user_type'=>2,
                    'mobile'=>$user_data['temp_user_phone_number'],
                    'body'=>$message
                ];
                // $this->whatsapp->send_message($whatsapp_data);
                if ($sening_sms) {
                ...
                if (strtotime($user_data['temp_auth_otp_expiry_time']) >= $this->utc_time) {
                    ...
                    if ($user_data['temp_auth_code'] == $otp) {
                        ...
                        $insert_user = array(
                            ...
                            'user_plan_id' => DEFAULT_DOCTOR_SUB_PLAN_ID,
                            'user_phone_verified' => 1
                        );
                        ...
                        //if user type is doctor then status is inactive
                        if ($user_type == 2) {
                            $insert_user['user_status'] = 3; //IN PENDING STATUS
                        ...
                        if ($insert_id > 0) {
                            assign_sub_plan($insert_id, DEFAULT_DOCTOR_SUB_PLAN_ID);
                            //save invoice detail
                            $doc_invoice_detail = array(
                                'doctor_id' => $insert_id,
                                'inv_prefix' => 'INV',
                                'inv_counter' => 0,
                                'created_at' => $this->utc_time_formated,
                            );
                            $this->Common_model->insert(TBL_INVOICE_DETAIL, $doc_invoice_detail);
                            ...
                            if ($token) {
                                ...
                                $user_data['is_sub_active'] = true;
                                $user_data['sub_plan_setting'] = doctor_sub_plan_setting($insert_id);
                                $this->my_response['status'] = true;
                        ...
                        if ($user_details['auth_code'] == $otp) {
                            ...
                            if (!empty($is_login) && $is_login == '1') {
                                ...
                                if ($user_type == 1) {
                                    ...
                                    $user_data['doctor_id'] = $user_details['user_id'];
                                    if(!empty($doctor_clinic_mapping['doctor_clinic_mapping_doctor_id'])) {
                                        $user_data['doctor_id'] = $doctor_clinic_mapping['doctor_clinic_mapping_doctor_id'];
                                    }
                                    if ($doctor_clinic_mapping) {
                                    ...
                                    $user_data['is_sub_active'] = is_sub_active($user_data['doctor_id']);
                                    $user_data['sub_plan_setting'] = doctor_sub_plan_setting($user_data['doctor_id']);
                                    $user_data['hour_format'] = get_hour_format($user_data['doctor_id']);

                                    if(!$user_data['is_sub_active'] && $user_data['user_type'] != 1) {
                                        $this->my_response = array(
                                            "status" => false,
                                            "message" => lang("subscription_expired_message")
                                        );
                                ...
                                //Get Doctor advertisement
                                $advertisement = $this->get_advertisement_data($user_data['user_id']);
                                if(count($advertisement) > 0) {
                                    $user_data['advertisement_data'] = $advertisement;
                                    $user_data['next_request_time'] = (strtotime($advertisement[0]['advertisement_end_date']) - time()) * 1000;
                                }
    ...
    public function resend_otp_post() {
        ...
        try {
            ...
            if (
                ...
                $send_otp = array(
                    'phone_number' => $phone_number,
                    'doctor_id' => ($user_details['doctor_clinic_mapping_role_id'] == 1 || empty($user_details['doctor_clinic_mapping_role_id'])) ? $user_details['user_id'] : $user_details['doctor_clinic_mapping_doctor_id'],
                    'message' => $message,
                    'whatsapp_sms_body' => sprintf(lang('wa_template_30_otp'), $otp)
                );
                $sening_sms = send_communication($send_otp);
    ...
    public function update_profile_post() {
        ...
        try {
            ...
            $getting_old_details = $this->Common_model->get_single_row(TBL_USERS, 'user_email,user_phone_number,user_email_verified', array('user_id' => $other_user_id));
            $flag = 1;
            ...
            if (!empty($email) && (empty($getting_old_details['user_email'])) || $getting_old_details['user_email_verified'] == 2 ) {
            ...
            if ($user_is_updated > 0 || $address_is_update > 0 || $user_details_is_update > 0) {
                ...
                if (!empty($emergency_contact_number)) {
                    ...
                    $send_otp = array(
                        ...
                        'doctor_id' => $user_id,
                        'is_sms_count' => true,
                        'is_check_sms_credit' => $GLOBALS['ENV_VARS']['CHECK_SMS_CREDIT']['OTP_MESSAGE']
                    );
                ...
                if (!empty($email) && $flag == 1 && (strtolower($getting_old_details['user_email']) != strtolower($email))) {
                    $reset_token = str_rand_access_token(20);
    ...
    public function upload_image_post() {
        try {
            ...
            if (!empty($_FILES['id_proof_file']['name']) && $_FILES['id_proof_file']['error'] == 0) {
                $new_profile_img = '';
                $upload_path = UPLOAD_REL_PATH . "/" . USER_ID_PROOF_FOLDER . "/" . $user_id;
                $upload_folder = USER_ID_PROOF_FOLDER . "/" . $user_id;
                $id_proof_image = do_upload_multiple($upload_path, array('id_proof_file' => $_FILES['id_proof_file']), $upload_folder);
                $id_proof_file_path = get_file_json_detail(USER_ID_PROOF_FOLDER . "/" . $user_id . "/" . $id_proof_image['id_proof_file']);
                if(!empty($id_proof_image['id_proof_file'])) {
                    $update_user_details = ['user_details_id_proof_image' => $id_proof_file_path];
                    $this->User_model->update_user_details($user_id, $update_user_details);
                    $this->my_response['status'] = true;
                    $this->my_response['message'] = lang("user_photo_upload_success");
                }
            }
            if (!empty($_FILES['photo']['name']) && $_FILES['photo']['error'] == 0) {
                if($_FILES['photo']['size'] >= (5000*1000)) { //5000*1000=5MB 
                        $this->my_response['status'] = false;
                        $this->my_response['message'] = lang("file_size_does_not_exist");
                        $this->send_response();
                        return;
                }
                ...
                if (!empty($new_profile_img)) {
                    ...
                    $update_data['user_photo_filepath'] = get_file_json_detail(USER_FOLDER . "/" . $user_id . "/" . $new_profile_img);
                    ...
                    if ($user_is_updated) {
                        ...
                        $get_user_details['user_photo_filepath_thumb'] = '';
                        if(!empty($get_user_details['user_photo_filepath']))
                            $get_user_details['user_photo_filepath_thumb'] = get_image_thumb($get_user_details['user_photo_filepath']);
                        $get_user_details['user_photo_filepath'] = get_file_full_path($get_user_details['user_photo_filepath']);
                        $this->my_response['status'] = true;
                        $this->my_response['message'] = lang("user_photo_upload_success");
    ...
    public function upload_sign_image_post() {
        try {
            $update_data = array();
            $sign_photo_base64 = !empty($this->post_data['sign_photo']) ? trim($this->post_data['sign_photo']) : "";
            $user_id = $this->user_id;
            if (empty($user_id) || empty($sign_photo_base64)) {
                $this->bad_request();
                exit;
            }
            $upload_path = UPLOAD_REL_PATH . "/" . USER_SIGN_FOLDER . "/" . $user_id;
            $upload_folder = USER_SIGN_FOLDER . "/" . $user_id;
            ...
            if($is_upload) {
                ...
                $this->my_response['status'] = true;
                $this->my_response['message'] = lang("user_photo_upload_success");
                $this->my_response['user_data'] = $update_data;
            } else {
                $this->my_response['status'] = false;
                $this->my_response['message'] = lang("user_photo_upload_fail");
            }
    ...
    public function get_family_members_post() {
        ...
        try {
            ...
            $get_family_members = $this->User_model->get_family_members($where);
    ...
    public function get_medical_condition_post() {
        try {
            $medical_condition_data = $this->Common_model->get_medical_condition();
            $this->my_response['status'] = true;
