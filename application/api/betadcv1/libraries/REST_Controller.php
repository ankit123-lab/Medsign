...
abstract class REST_Controller extends \CI_Controller {
    ...
    public function __construct($config = 'rest') {
        ...
        $api_class = $this->router->fetch_class();
        $api_method = $this->router->fetch_method();

        if (!in_array($api_class . "/" . $api_method, $this->config->item('except_uris'))) {
    ...
    public function response($data = NULL, $http_code = NULL, $continue = false) {
        ...
        $output = NULL;
        $data['request_id'] = $this->_insert_id;
        ...
        //$output['request_id'] = $this->_insert_id;
        // Output the data
    ...
    protected function _log_request($authorized = FALSE) {
        ...
        $deviceType = '';
        if (isset($this->_args['device_type'])) {
            if ($this->_args['device_type'] == 'ios') {
                $deviceType = 'ios';
            } else if ($this->_args['device_type'] == 'android') {
                $deviceType = 'android';
            } else {
                $deviceType = 'web';
            }
        ...
        $is_inserted = $this->rest->db
            ...
            'al_api_name' => $this->uri->uri_string(),
            'al_api_method' => $this->request->method,
            'al_request_data' => $this->_args ? ($this->config->item('rest_logs_json_params') === TRUE ? json_encode($this->_args) : json_encode($this->_args)) : NULL,
            //'api_key' => isset($this->rest->key) ? $this->rest->key : '',
            'al_device_type' => $deviceType,
            'al_ip_address' => $this->input->ip_address(),
            'al_created_date' => time(),
            'al_authorized' => $authorized,
            'al_version' => API_VERSION
        ]);
    ...
    protected function _log_access_time() {
        $payload['al_processing_time'] = $this->_end_rtime - $this->_start_rtime;
        if (is_array($this->response_data)) {
            $payload['al_response_data'] = json_encode($this->response_data);
        } else {
            $payload['al_response_data'] = $this->response_data;
        }
        return $this->rest->db->update(
                        $this->config->item('rest_logs_table'), $payload, [
                    'al_id' => $this->_insert_id
        ]);
    }
    ...
    protected function _log_response_code($http_code) {
        $payload['al_response_code'] = $http_code;
