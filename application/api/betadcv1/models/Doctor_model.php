...
class Doctor_model extends MY_Model {
     ...
     * @param type $files
     */
    public function add_specialization($user_id, $specialization_json, $files) {
        ...
        //insert new data
        $specialization_data = json_decode($specialization_json, true);
        $insert_array 		 = array();
        
		//upload files
		if(isset($files) && isset($files['specialization_images']) && !empty($files['specialization_images'])){
        ...
        foreach ($specialization_data as $key => $value) {
            $insert_array[] = array(
                "doctor_specialization_doctor_id" 		=> $user_id,
                "doctor_specialization_specialization_id" => $value['id'],
                "doctor_specialization_specialization_text" => $value['title'],
                "doctor_specialization_image" 			=> (isset($profile_image_name) && isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? $profile_image_name[$key] : '',
                "doctor_specialization_image_full_path" => (isset($profile_image_name) && isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? (IMAGE_MANIPULATION_URL . DOCTOR_SPECIALIZATION_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key]) : '',
    ...
    public function add_educations_details($user_id, $educations_json, $files) {
        ...
        //upload files
		if(isset($files) && isset($files['education_images']) && !empty($files['education_images'])){
        ...
        foreach ($educations_array as $key => $value) {
            $insert_array[] = array(
                ...
                "doctor_qualification_image_full_path" => (isset($profile_image_name) && isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? (get_file_json_detail(DOCTOR_EDUCATIONS_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key])) : '',
    ...
    public function add_registrations_details($user_id, $registration_json, $files) {
        ...
        //upload files
		if(isset($files) && isset($files['registration_images']) && !empty($files['registration_images'])){
        ...
        foreach ($registration_array as $key => $value) {
            $insert_array[] = array(
                ...
                "doctor_registration_image" 		 => (isset($file_names) && isset($file_names[$key]) && !empty($file_names[$key])) ? $file_names[$key] : '',
                "doctor_registration_image_filepath" => (isset($file_names) && isset($file_names[$key]) && !empty($file_names[$key])) ? (get_file_json_detail(DOCTOR_REGISTRATIONS_FOLDER . "/" . $user_id . "/" . $file_names[$key])) : '',
    ...
    public function get_doctor_search_list($user_id, $filter_array = array(), $is_for_total_count = false) {
        ...
        if (in_array(3, $filter_array['filter_option'])) {
            ...
            LEFT JOIN " . TBL_ADDRESS . " 
                ON address_user_id=clinic_id AND address_status=1 AND address_type=2
                ";
        }
        ...
        if (!empty($filter_array['appointment_type'])) {
            $doctor_query.=" JOIN " . TBL_DOCTOR_AVAILABILITY . "
                ...
                AND (doctor_availability_appointment_type =  '" . $filter_array['appointment_type'] . "' OR doctor_availability_appointment_type = 1) 
    ...
    public function doctor_detail($doctor_id, $clinic_id, $flag = '', $appointment_type = '') {
        ...
        if (!empty($appointment_type)) {
            $doctor_query .= "
                LEFT JOIN " . TBL_DOCTOR_AVAILABILITY . "
                    ...
                    AND (doctor_availability_appointment_type  = '" . $appointment_type . "' OR doctor_availability_appointment_type = 1)  ";
        }
        ...
        $doctor_query .= "
            ...
            LEFT JOIN " . TBL_DOCTOR_CLINIC_MAPPING . " 
                ON doctor_clinic_mapping_user_id=user_id AND doctor_clinic_mapping_status=1
	...
	public function doctor_basic_details($doctor_id, $clinic_id) {
        $doctor_query = "
             SELECT
                user_id,
                user_first_name,
    ...
    public function get_doctor_availibility($check_availability = array()) {
        ...
        $timeslots_data = $this->get_all_rows_by_query($get_availability_sql);
	...
	public function get_all_block_calender($request) {
        $calender_block_query = "
			 SELECT
				calender_block_id,
				calender_block_from_date,
     ...
     * @return type
     */
    public function get_doctor_edu_detail($doctor_id, $flag = false, $columns = '*') {
        $doctor_query = "
             SELECT
                " . $columns . "                              
            FROM 
        ...
        foreach ($doctor_data as $key => $value) {
            $doctor_data[$key]['image_thumb_path'] = '';
            if(!empty($value['doctor_qualification_image_full_path']))
                $doctor_data[$key]['image_thumb_path'] = get_image_thumb($value['doctor_qualification_image_full_path']);
            $doctor_data[$key]['doctor_qualification_image_full_path'] = get_file_full_path($value['doctor_qualification_image_full_path']);
        }
        return $doctor_data;
    }
     ...
     * @return type
     */
    public function get_doctor_specialisation_detail($doctor_id, $flag = false, $columns = '*') {
        $doctor_query = "
            SELECT
                " . $columns . "                              
            FROM 
     ...
     * @return type
     */
    public function get_doctor_reg_detail($doctor_id, $flag = false, $columns = '*') {
        $doctor_query = "
            SELECT
                " . $columns . "                              
            FROM 
        ...
        foreach ($doctor_data as $key => $value) {
            $doctor_data[$key]['image_thumb_path'] = '';
            if(!empty($value['doctor_registration_image_filepath']))
                $doctor_data[$key]['image_thumb_path'] = get_image_thumb($value['doctor_registration_image_filepath']);
                $doctor_data[$key]['doctor_registration_image_filepath'] = get_file_full_path($value['doctor_registration_image_filepath']);
        }
        return $doctor_data;
    }
     ...
     * @return type
     */
    public function get_doctor_award_detail($doctor_id, $columns = '*') {
        $doctor_query = "
            SELECT
                " . $columns . "                              
            FROM 
        ...
        foreach ($doctor_data as $key => $value) {
            $doctor_data[$key]['image_thumb_path'] = '';
            if(!empty($value['doctor_award_image_fullpath']))
                $doctor_data[$key]['image_thumb_path'] = get_image_thumb($value['doctor_award_image_fullpath']);
            $doctor_data[$key]['doctor_award_image_fullpath'] = get_file_full_path($value['doctor_award_image_fullpath']);
        }
        return $doctor_data;
    }
    ...
    public function update_specialisation_data($user_id, $specialisation_array, $files) {
        $insert_array = array();
        if (isset($files) && !empty($files) && isset($files['specialization_images'])) {
            $upload_path = UPLOAD_REL_PATH . "/" . DOCTOR_SPECIALIZATION_FOLDER . "/" . $user_id;
            $upload_folder = DOCTOR_SPECIALIZATION_FOLDER . "/" . $user_id;
        ...
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_specialization_id,doctor_specialization_doctor_id,doctor_specialization_specialization_id,doctor_specialization_specialization_text,doctor_specialization_image_full_path';
            $get_specialisation = $this->get_doctor_specialisation_detail($user_id,false,$columns);
            $specialisation_old = array();
            foreach ($get_specialisation as $key => $value) {
                $specialisation_old[$value['doctor_specialization_id']] = $value;
            }
        }
        $doctor_where = array(
            "doctor_specialization_doctor_id" => $user_id,
        ...
        foreach ($specialisation_array as $key => $sp) {
            if (empty($sp['doctor_specialization_id'])) {
                $insert_array[] = array(
                    ...
                    "doctor_specialization_specialization_text" => $sp['doctor_specialization_specialization_text'],
                    "doctor_specialization_image" => (isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? $profile_image_name[$key] : '',
					"doctor_specialization_image_full_path" => (isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? (IMAGE_MANIPULATION_URL . DOCTOR_SPECIALIZATION_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key]) : '',
                    "doctor_specialization_created_at" => $this->utc_time_formated,
                    "doctor_specialization_status" => 1,
                );
            } else {
        ...
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_specialization_id,doctor_specialization_doctor_id,doctor_specialization_specialization_id,doctor_specialization_specialization_text,doctor_specialization_image_full_path';
            $get_specialisation = $this->get_doctor_specialisation_detail($user_id,false,$columns);
            $specialisation_new = array();
            foreach ($get_specialisation as $key => $value) {
                if(empty($value['doctor_specialization_specialization_text'])) {
                    $value['doctor_specialization_specialization_text'] = '';
                }
                $specialisation_new[$value['doctor_specialization_id']] = $value;
            }
            $this->load->model("Auditlog_model");
            $this->Auditlog_model->create_audit_log($user_id, 2, AUDIT_SLUG_ARR['USER_PROFILE_ACTION'], $specialisation_old, $specialisation_new, TBL_DOCTOR_SPECIALIZATIONS, 'doctor_specialization_doctor_id', $user_id);
        }
    ...
    public function update_education_data($user_id, $education_qualification_array, $files) {
        $insert_array = array();
        if (isset($files) && !empty($files) && isset($files['education_images'])) {
            $upload_path = UPLOAD_REL_PATH . "/" . DOCTOR_EDUCATIONS_FOLDER . "/" . $user_id;
            $upload_folder = DOCTOR_EDUCATIONS_FOLDER . "/" . $user_id;
        ...
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_qualification_id,doctor_qualification_user_id,doctor_qualification_degree,doctor_qualification_college,doctor_qualification_completion_year,doctor_qualification_image_full_path';
            $get_doctor_edu_detail = $this->get_doctor_edu_detail($user_id,false,$columns);
            $doctor_edu_detail_old = array();
            foreach ($get_doctor_edu_detail as $key => $value) {
                $doctor_edu_detail_old[$value['doctor_qualification_id']] = $value;
            }
        }
        $this->update(TBL_DOCTOR_EDUCATIONS, $update_array, $doctor_where);
        $new_education_qualification = array();
        foreach ($education_qualification_array as $key => $edu) {
            if (empty($edu['doctor_qualification_id'])) {
                $insert_array[] = array(
                    ...
                    "doctor_qualification_completion_year" => (!empty($edu['doctor_qualification_completion_year'])) ? $edu['doctor_qualification_completion_year'] : NULL,
                    "doctor_qualification_qualification_id" => (!empty($edu['doctor_qualification_qualification_id'])) ? $edu['doctor_qualification_qualification_id'] : 0,
                    "doctor_qualification_image" => (isset($profile_image_name) && isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? $profile_image_name[$key] : '',
					"doctor_qualification_image_full_path" => (isset($profile_image_name) && isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? (get_file_json_detail(DOCTOR_EDUCATIONS_FOLDER."/".$user_id."/".$profile_image_name[$key])) : '',
                    "doctor_qualification_created_at" => $this->utc_time_formated,
                    "doctor_qualification_status" => 1,
                ...
                $update_array = array(
                    ...
                    "doctor_qualification_college" => $edu['doctor_qualification_college'],
                    "doctor_qualification_completion_year" => (!empty($edu['doctor_qualification_completion_year'])) ? $edu['doctor_qualification_completion_year'] : NULL,
                    "doctor_qualification_qualification_id" => (!empty($edu['doctor_qualification_qualification_id'])) ? $edu['doctor_qualification_qualification_id'] : NULL,
                    "doctor_qualification_modified_at" => $this->utc_time_formated,
                    "doctor_qualification_status" => 1
                );
                if (isset($profile_image_name) && isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) {
                    $update_array['doctor_qualification_image_full_path'] = get_file_json_detail(DOCTOR_EDUCATIONS_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key]);
        ...
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_qualification_id,doctor_qualification_user_id,doctor_qualification_degree,doctor_qualification_college,doctor_qualification_completion_year,doctor_qualification_image_full_path';
            $get_doctor_edu_detail = $this->get_doctor_edu_detail($user_id,false,$columns);
            $doctor_edu_detail_new = array();
            foreach ($get_doctor_edu_detail as $key => $value) {
                $doctor_edu_detail_new[$value['doctor_qualification_id']] = $value;
            }
            $this->load->model("Auditlog_model");
            $this->Auditlog_model->create_audit_log($user_id, 2, AUDIT_SLUG_ARR['USER_PROFILE_ACTION'], $doctor_edu_detail_old, $doctor_edu_detail_new, TBL_DOCTOR_EDUCATIONS, 'doctor_qualification_user_id', $user_id);
        }
    ...
    public function update_registration_data($user_id, $registration_details_array, $files) {
        $insert_array = array();
        if (isset($files) && isset($files['registration_images'])) {
            $upload_path = UPLOAD_REL_PATH . "/" . DOCTOR_REGISTRATIONS_FOLDER . "/" . $user_id;
            $upload_folder = DOCTOR_REGISTRATIONS_FOLDER . "/" . $user_id;
            $profile_image_name = do_upload_multiple3($upload_path, array('photo' => $files['registration_images']), $upload_folder);
        }
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_registration_id,doctor_registration_user_id,doctor_registration_council_id,doctor_council_registration_number,doctor_registration_year,doctor_registration_image_filepath';
            $get_doctor_reg_detail = $this->get_doctor_reg_detail($user_id,false,$columns);
            $doctor_reg_detail_old = array();
            foreach ($get_doctor_reg_detail as $key => $value) {
                $doctor_reg_detail_old[$value['doctor_registration_id']] = $value;
            }
        }
        $doctor_where = array(
            "doctor_registration_user_id" => $user_id,
        ...
        foreach ($registration_details_array as $key => $reg) {
            if (empty($reg['doctor_registration_id'])) {
                $insert_array[] = array(
                    ...
                    "doctor_registration_image_filepath" => (isset($profile_image_name) && isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) ? (get_file_json_detail(DOCTOR_REGISTRATIONS_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key])) : '',
                ...
                if (isset($profile_image_name[$key]) && !empty($profile_image_name[$key])) {
                    $update_array['doctor_registration_image_filepath'] = get_file_json_detail(DOCTOR_REGISTRATIONS_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key]);
        ...
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_registration_id,doctor_registration_user_id,doctor_registration_council_id,doctor_council_registration_number,doctor_registration_year,doctor_registration_image_filepath';
            $get_doctor_reg_detail = $this->get_doctor_reg_detail($user_id,false,$columns);
            $doctor_reg_detail_new = array();
            foreach ($get_doctor_reg_detail as $key => $value) {
                $doctor_reg_detail_new[$value['doctor_registration_id']] = $value;
            }
            $this->load->model("Auditlog_model");
            $this->Auditlog_model->create_audit_log($user_id, 2, AUDIT_SLUG_ARR['USER_PROFILE_ACTION'], $doctor_reg_detail_old, $doctor_reg_detail_new, TBL_DOCTOR_REGISTRATIONS, 'doctor_registration_user_id', $user_id);
        }
    }
    ...
    public function update_award_data($user_id, $award_details_array, $files) {
        ...
        $insert_array = array();
        if (isset($files) && isset($files['award_images'])) {
        ...
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_award_id,doctor_award_doctor_id,doctor_award_name,doctor_award_year,doctor_award_image_fullpath';
            $get_award = $this->get_doctor_award_detail($user_id,$columns);
            $doctor_award_old = array();
            foreach ($get_award as $key => $value) {
                $doctor_award_old[$value['doctor_award_id']] = $value;
            }
        }
        $doctor_where = array(
        ...
        foreach ($award_details_array as $key => $reg) {
            if (empty($reg['doctor_award_id'])) {
                $insert_array[] = array(
                    ...
                    "doctor_award_image_fullpath" => (!empty($profile_image_name[$key])) ? (get_file_json_detail(DOCTOR_AWARDS_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key])) : '',
                    "doctor_award_image" => (!empty($profile_image_name[$key])) ? $profile_image_name[$key] : '',
                ...
                if (!empty($profile_image_name[$key])) {
                    $update_array['doctor_award_image_fullpath'] = get_file_json_detail(DOCTOR_AWARDS_FOLDER . "/" . $user_id . "/" . $profile_image_name[$key]);
        ...
        if(IS_AUDIT_LOG_ENABLE) {
            $columns = 'doctor_award_id,doctor_award_doctor_id,doctor_award_name,doctor_award_year,doctor_award_image_fullpath';
            $get_award = $this->get_doctor_award_detail($user_id,$columns);
            $doctor_award_new = array();
            foreach ($get_award as $key => $value) {
                $doctor_award_new[$value['doctor_award_id']] = $value;
            }
            $this->load->model("Auditlog_model");
            $this->Auditlog_model->create_audit_log($user_id, 2, AUDIT_SLUG_ARR['USER_PROFILE_ACTION'], $doctor_award_old, $doctor_award_new, TBL_DOCTOR_AWARDS, 'doctor_award_doctor_id', $user_id);
        }
    }
    ...
    public function appointment_exists_block_calendar($data = array()) {
        ...
        $get_appointment_list_sql = " SELECT 
                                                appointment_id, appointment_user_id, appointment_from_time, appointment_to_time, appointment_date, 
                                                clinic_id,
                                                clinic_name,
                                                address_name, 
                                                address_name_one, 
                                          ...
                                          FROM 
                                                " . TBL_APPOINTMENTS . " 
                                            JOIN " . TBL_CLINICS . " ON appointment_clinic_id = clinic_id
                                            LEFT JOIN " . TBL_ADDRESS . " 
                                                ON address_user_id=clinic_id AND address_type=2 
                                            LEFT JOIN " . TBL_CITIES . " 
                                                ON address_city_id = city_id 
        ...
        if ($data['block_type'] == 1) {
            ...
            $get_appointment_list_sql .= "
                                            (
                                                ( appointment_from_time >= '" . $data['start_time'] . "' AND appointment_from_time < '" . $data['end_time'] . "' ) OR 
												( appointment_to_time > '" . $data['start_time'] . "' AND appointment_to_time <= '" . $data['end_time'] . "' ) 
                                            )
    ...
    public function get_doctor_patient_list($requested_data) {
        $join_query = " AND appointment_doctor_user_id = '" . $requested_data['doctor_id'] . "'
                       AND appointment_clinic_id = '" . $requested_data['clinic_id'] . "'";
        if ($requested_data['patient_list_type'] == 1 || $requested_data['patient_list_type'] == 2) {
            $join_query .= " AND appointment_date = CURDATE() ";
        }
        if ($requested_data['patient_list_type'] == 2) {
            $join_query .= " AND is_done=0 ";
        }
        $columns = "u.user_id, 
                    u.user_first_name, 
                    u.user_last_name, 
					...
					u.user_email,
                    a1.appointment_id,
                    a1.appointment_doctor_user_id,
                    a1.appointment_clinic_id,
                    a1.is_done,
                    CONCAT(a1.appointment_date,' ',a1.appointment_from_time) AS appointment_datetime,
                    CONCAT(u.user_first_name,' ',u.user_last_name) AS user_name ";

        $get_patient_list_query = " SELECT " . $columns . " 
                                    FROM 
                                            " . TBL_APPOINTMENTS . "  a1
                                    JOIN (SELECT appointment_user_id, MAX(appointment_id) appointment_id 
                                    FROM me_appointments where appointment_status != 9 " . $join_query . "
                                    GROUP BY appointment_user_id) a2
                                    ON a1.appointment_id = a2.appointment_id 
                                    AND a1.appointment_user_id = a2.appointment_user_id

                                    LEFT JOIN
                                            " . TBL_USERS . " u ON a1.appointment_user_id = u.user_id 
                                    WHERE
                                            u.user_status  = 1
                                    AND 
                                            a1.appointment_doctor_user_id = '" . $requested_data['doctor_id'] . "' 
                                    AND
                                            a1.appointment_clinic_id = '" . $requested_data['clinic_id'] . "' ";

        if ($requested_data['patient_list_type'] == 2) {
            $get_patient_list_query .= " AND a1.is_done=0 ";
        }
        if ($requested_data['patient_list_type'] == 1 || $requested_data['patient_list_type'] == 2) {
            $get_patient_list_query .= " AND a1.appointment_date = CURDATE() ";
            $get_patient_list_query .= " ORDER BY appointment_datetime asc ";
        } else {
            $get_patient_list_query .= " ORDER BY a1.appointment_id desc ";
        }
...
// echo $get_patient_list_query;die;
    ...
    public function get_search_doctor($requested_data) {
        ...
        $get_doctor_detail_query .=" GROUP BY user_id ";
	...
	/**
     * Description :- This function helps to get the all the future appointments from the curdate time and check each appointment time based on the doctor  availability slots newly defined 
     * 
     * @author MedEasy
     * 
     * @param type $data
     * @return type
     */
    public function get_future_appointment_exists($data = array()) {
		//get the appointment list if exists in the date
        $get_appointment_list_sql = "SELECT appointment_id, appointment_user_id, appointment_from_time, appointment_to_time, appointment_date,clinic_id,
                                                clinic_name,
                                                address_name, 
                                                address_name_one, 
                                                address_locality, 
										  ...
										  AND 
                                                appointment_doctor_user_id = '" . $data['doctor_id'] . "' 
										  AND 
                                                appointment_clinic_id = '" . $data['clinic_id'] . "' 
                                          AND CONCAT(appointment_date,' ',appointment_from_time) >='".get_display_date_time('Y-m-d H:i:s')."'";
		$get_appointment_data = $this->Common_model->get_all_rows_by_query($get_appointment_list_sql);
		return $get_appointment_data;
    }

    /**
     * Description :- This function is used to get the specialisation and education ids of doctor
     * 
     * @param type $doctor_id
     * @param type $flag
     * @return type
     */
    public function get_doctor_specialisation_edu_ids($doctor_id) {
        $doctor_query = "
            select user_id,
            group_concat(DISTINCT doctor_specialization_specialization_id SEPARATOR '|') as doctor_specialization_specialization_ids, 
            group_concat(DISTINCT doctor_qualification_qualification_id SEPARATOR '|') as doctor_qualification_qualification_ids 
            from me_users 
            left join me_doctor_specializations on doctor_specialization_doctor_id=user_id and doctor_specialization_status !=9 
            left join me_doctor_qualifications on doctor_qualification_user_id=user_id and doctor_qualification_status !=9
            where user_id = " . $doctor_id . " group by user_id   
            ";
            return $this->get_single_row_by_query($doctor_query);
    }

    public function get_ds_template($where, $count = false) {
        $this->db->select("
            template_id,
            template_template_name, 
            template_diagnosis_name, 
            template_investigation_name,
            GROUP_CONCAT(DISTINCT(drug_name)) as drug_name,
            GROUP_CONCAT(DISTINCT(clinical_notes_catalog_title) SEPARATOR ', ') as clinical_notes_title
        ");
        $this->db->from(TBL_TEMPLATE);
        $this->db->join(TBL_TEMPLATE_DRUG, 'template_id = template_drug_binding_template_id', "LEFT");
        $this->db->join(TBL_DRUGS, 'template_drug_binding_drug_id = drug_id', "LEFT");
        $this->db->join(TBL_CLINICAL_NOTES, 'FIND_IN_SET(clinical_notes_catalog_id, template_clinical_notes_id) AND clinical_notes_catalog_status=1', "LEFT");
        $this->db->where('template_doctor_id', $where['doctor_id']);
        $this->db->where('template_drug_binding_status', 1);
        $this->db->where('template_status', 1);
        if(!empty($where['search']))
            $this->db->like('template_diagnosis_name', $where['search']);
