...
class Clinic_model extends MY_Model {
     ...
     * @return type
     */
    public function get_clinic_detail($clinic_id, $column = '') {
    ...
    public function get_clinic_whole_detail($clinic_id, $doctor_id) {
        $clinic_query = "
            SELECT
                ...
                doctor_clinic_mapping_tele_fees,
                clinic_filepath,
                clinic_services,
                clinic_phone_verified,
                clinic_email_verified,
    ...
    public function get_staff_details($request_data) {
        ...
        $columns = "doctor_clinic_mapping_user_id as user_id,
                    ...
                    GROUP_CONCAT(clinic_name) AS clinic_name,
                    user_first_name, 
                    user_last_name, 
...
/*
        ...
        */
        ...
        $this->db->from(TBL_DOCTOR_CLINIC_MAPPING);
        $this->db->join(TBL_USERS, 'doctor_clinic_mapping_user_id = user_id');
        $this->db->join(TBL_ADDRESS, 'address_user_id = user_id AND address_type = 1', "LEFT");
        $this->db->join(TBL_CLINICS, 'clinic_id = doctor_clinic_mapping_clinic_id');
        $this->db->where('(doctor_clinic_mapping_doctor_id = '.$request_data['doctor_id'].' OR doctor_clinic_mapping_user_id = '.$request_data['doctor_id'].')');
        $this->db->where('user_status !=', 9);
        $this->db->where('doctor_clinic_mapping_status !=', 9);
        $this->db->group_by('doctor_clinic_mapping_user_id');
        $query = $this->db->get();
        return $query->result_array();
    }

    public function get_doctor_global_setting($doctor_id) {
        $this->db->select('*')->from('me_doctors_global_setting dgs');
        $this->db->join(TBL_USERS . ' u', 'u.user_id=dgs.doctor_id AND u.user_plan_id=dgs.sub_plan_id');
        $this->db->where('dgs.setting_status', 1);
        $this->db->where('dgs.doctor_id', $doctor_id);
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function get_clinic_list($doctor_id) {
        $clinic_query = "
                        ...
                        LEFT JOIN
                            me_settings 
                        ON 
                            setting_clinic_id=B.doctor_clinic_mapping_clinic_id AND 
                            setting_type=13 AND setting_status=1
                        WHERE 
                            clinic_status=1";
        $clinic_data = $this->get_all_rows_by_query($clinic_query);
        return $clinic_data;
	...
	public function get_clinic_availability($clinic_id){
		if(empty($clinic_id))
			return [];
		...
		$this->db->select($clinic_columns)->from(TBL_CLINIC_AVAILABILITY);
        $this->db->where('clinic_availability_status !=', 9);
        if(is_array($clinic_id))
            $this->db->where_in('clinic_availability_clinic_id', $clinic_id);
        else
        	$this->db->where('clinic_availability_clinic_id', $clinic_id);				
        $query = $this->db->get();
        return $query->result_array();
		...
		// );
  //       $getClinicAvailability = $this->get_all_rows(TBL_CLINIC_AVAILABILITY, $clinic_columns, $clinic_where_availability);
		// return $getClinicAvailability;
	}
    public function get_doctor_availability($clinic_id, $doctor_id) {
        if(empty($clinic_id))
            return [];
        ...
        $this->db->select($columns)->from(TBL_DOCTOR_AVAILABILITY);
        $this->db->where('doctor_availability_status !=', 9);
        $this->db->where('doctor_availability_user_id', $doctor_id);
        if(is_array($clinic_id))
            $this->db->where_in('doctor_availability_clinic_id', $clinic_id);
        else
            $this->db->where('doctor_availability_clinic_id', $clinic_id);              
        $query = $this->db->get();
        return $query->result_array();
        // $clinic_where_availability = array(
        //  'clinic_availability_clinic_id' => $clinic_id,
    ...
    public function getStaffWholeDetail($params) {
        if ($params) {
            $this->db->select(array(
                ...
                'GROUP_CONCAT(cm.doctor_clinic_mapping_clinic_id) AS clinic_id',
            ));

            $this->db->from(TBL_USERS);
            $this->db->join(TBL_ADDRESS, 'address_user_id = user_id AND address_type = 1', 'left');
            $this->db->join('me_doctor_clinic_mapping cm', 'cm.doctor_clinic_mapping_user_id = user_id AND cm.doctor_clinic_mapping_status=1', 'left');
