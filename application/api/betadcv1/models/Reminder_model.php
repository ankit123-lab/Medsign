...
class Reminder_model extends MY_Model {
    ...
    public function get_sync_reminders($user_id, $sync_date, $reminder_type) {
        ...
        if (!empty($sync_date)) {
            $reminder_query.="
                    AND
                    (
                        reminder_created_at >='" . $sync_date . "' OR
                        reminder_modified_at >='" . $sync_date . "' 
                    )
            ";
        } else {
            $reminder_query.=" 
    ...
    public function get_chart_data($user_id, $date) {
        ...
        $prev = date('Y-m-d', strtotime($date . '-30 days'));
        $query = "
                SELECT 
                    count(reminder_record_id) as take_count,   
                    reminder_record_id,                    
                    DAY(reminder_record_date) as reminder_day, 
                    group_concat(reminder_record_date ORDER BY reminder_record_date ASC) AS reminder_record_date_group,
